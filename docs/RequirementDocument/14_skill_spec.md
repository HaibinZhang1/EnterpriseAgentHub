...
- `README.md` 为市场详情页默认展示材料；客户端在详情覆盖层底部展示其完整 Markdown 内容，缺失时显示“暂无 README.md”
- `scripts/` 中的脚本默认视为可执行资源；当前版本 不做风险脚本扫描，仅保留文件并展示服务端已有风险字段
- `resources/` 中的内容默认视为说明或模板材料
- 用户本地未发布的 skill 无需包含 manifest.json，系统仍可通过扫描 SKILL.md 识别
- 当前版本 单个 Skill 包最大 5MB，包内文件最多 100 个
...
> 说明：当前版本 必须先固化客户端安装和市场展示所需的最小字段集；完整治理字段可随 后续版本 发布/审核能力补齐。当前版本 最小字段建议包括：`skillID`、显示名称、描述、当前版本、包下载地址或包标识、包 Hash、作者/部门展示信息、更新时间、适用工具、适用系统、公开级别、授权/可安装结果、当前状态、风险等级（可为空）、README/图标等展示资源地址。
...
### 当前版本 最小字段约束
- 当前版本 客户端不得依赖发布/审核表单字段才能完成安装与启用
- 当前版本 客户端必须能基于服务端返回的包地址/包标识与 Hash 完成下载、校验和覆盖更新
- 当前版本 客户端必须校验服务端返回的包大小和文件数，不接受超过 5MB 或超过 100 个文件的 Skill 包
- 当前版本 服务端负责校验固定中文分类 / 标签体系；不支持自由标签、标签审核、别名合并或标签管理后台
- 当前版本 可展示风险等级与审核摘要，但不生成、修改或通过脚本扫描推导这些治理字段

---

## 9.5 上传校验规则
上传后系统至少执行以下校验。当前版本 若通过管理员导入或后端预置入库，只要求执行 当前版本 最小入库校验；完整发布审核校验进入 后续版本。
...
- 必填字段完整
- `skill_id` 合法且未冲突
- `skill_id` 仅允许小写字母、数字和单个连字符，不能以连字符开头或结尾
- `version` 格式正确（SemVer）
- `description` 已填写
- 首次发布和版本更新必须填写 `changelog`
- `category` 必须属于固定中文短分类且恰好 1 个
- `tags` 必须属于固定中文短标签，数量 1 到 5 个，且不得重复
- `compatible_tools` 与 `compatible_systems` 不为空

### 内容校验
- README 可正常展示
- 图标、截图、示例文件类型合法
- 后续版本 可识别脚本文件并标记风险；当前版本 不做风险脚本扫描

### 安全初审校验
> 说明：安全初审校验进入 后续版本；当前版本 不执行风险脚本扫描。
...
- 包内文件数超过 100
- 公开级别非法
- 授权范围非法或选择“指定部门”但未选择任何部门

分类、标签、描述、变更说明等元数据缺失或非法时不得提交；如果服务端收到非法数据，返回校验失败并要求发布者修改后重新提交。
